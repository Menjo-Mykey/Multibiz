...
export const Dashboard: React.FC = () => {
  ...
  const fetchDashboardData = async () => {
    ...
    try {
      ...
      const totalRevenue =
        salesData?.reduce((sum, sale) => sum + Number(sale.total_amount), 0) ||
        0;
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-bold tracking-tight">Multibiz</h1>
          <p className="text-muted-foreground">
            Your all-in-one POS — welcome back! Here's what's happening with{" "}
      ...
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
        ...
        />
        <StatsCard title="Active Staff" value={data.activeStaff} icon={Users} />
        <StatsCard
      ...
      <div className="grid gap-6 md:grid-cols-2">
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              Latest transactions from {selectedBusiness.name}
            </CardDescription>
          </CardHeader>
          <CardContent>
                      ...
                      <div
                        className={`h-10 w-10 rounded-full flex items-center justify-center ${
                          selectedBusiness.type === "triplek"
                            ? "bg-triplek text-triplek-foreground"
                            : "bg-swan text-swan-foreground"
                        }`}
                      >
                        {selectedBusiness.type === "triplek" ? (
            ...
            {data.lowStockItems.length === 0 ? (
              ...
              <div className="space-y-3">
                {data.lowStockItems.map((item) => (
                  <div
                    key={item.id}
                    className="flex items-center justify-between p-3 border border-warning/20 rounded-lg bg-warning/5"
                  >
                    <div>
                      <p className="font-medium">{item.name}</p>
                      <p className="text-sm text-muted-foreground">
                        Current: {item.stock_quantity} • Threshold:{" "}
